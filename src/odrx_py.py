...
class APIClient:
    ...
    async def get_recent_score(self, user_id: int, offset: int = 0) -> list[Score]:
        req = await self.api.get(self.endpoints["get_recent"]+f"?id={user_id}&offset={offset}")

        return utils.parser.score(req)
    
    async def get_scores_uid_beatmap_fromid(self, user_id: int, map_id: int) -> Beatmap:
        utils.helpers.check_get_user_attrs(uid=user_id)
        req = await self.api.get(self.endpoints["beatmap_score"]+f"?id={map_id}&uid={user_id}")

        return utils.parser.beatmap(req)
    
    async def get_scores_uid_beatmap_frommd5(self, user_id: int, md5: str) -> Beatmap:
        utils.helpers.check_get_user_attrs(uid=user_id)
        req = await self.api.get(self.endpoints["beatmap_score"]+f"?md5={md5}&uid={user_id}")

        return utils.parser.beatmap(req)
    
    async def get_scores_uname_beatmap_fromid(self, username: str, map_id: int) -> Beatmap:
        utils.helpers.check_get_user_attrs(username=username)
        req = await self.api.get(self.endpoints["beatmap_score"]+f"?id={map_id}&username={username}")

        return utils.parser.beatmap(req)
    
    async def get_scores_uname_beatmap_frommd5(self, username: str, md5: str) -> Beatmap:
        utils.helpers.check_get_user_attrs(username=username)
        req = await self.api.get(self.endpoints["beatmap_score"]+f"?md5={md5}&username={username}")

        return utils.parser.beatmap(req)
    
    async def get_whitelist(self) -> list[Beatmap]: # Shit takes a long time to succeed, if it even does at all LMAO.
    ...
    async def whitelist_add_fromid(self, map_id: int) -> Beatmap:
        ...
        bmap = await self.api.get(self.endpoints["whitelist_add"]+f"?id={map_id}&key={self.key}")
